...
public class AutoStartLevel1SideCargoFrontSide1 extends CommandGroup {
  ...
  public AutoStartLevel1SideCargoFrontSide1() {
    ...
    addSequential(new DriveMotionCommand(TrajectoryGenerator.getInstance().getTrajectorySet().cargoFrontToTurn1A, false));

    addParallel(new AutoCameraTrackWhenCrossXBoundary(60, MovingXDirection.Negative, 0.7));
    addSequential(new DriveMotionCommand(TrajectoryGenerator.getInstance().getTrajectorySet().cargoFrontTurn1AToLoading, false), 4);
    addSequential(new IntakeHatchArms(HatchArmState.IN));
    addSequential(new WaitCommand("Grab Break", .25));

    addSequential(new DriveMotionCommand(TrajectoryGenerator.getInstance().getTrajectorySet().loadingToTurn1CargoSide1, false));
    addParallel(new AutoCameraTrackWhenCrossYBoundary(-70, MovingYDirection.OutsideToInside));
    addSequential(new DriveMotionCommand(TrajectoryGenerator.getInstance().getTrajectorySet().turn1ToCargoSide1, false));
