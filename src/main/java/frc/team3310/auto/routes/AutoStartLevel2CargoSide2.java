...
public class AutoStartLevel2CargoSide2 extends CommandGroup {
  /**
   * Add your docs here.
   */
  public AutoStartLevel2CargoSide2() {
    ...
    addSequential(new DriveMotionCommand(TrajectoryGenerator.getInstance().getTrajectorySet().driveStraight, true));
    addSequential(
        new DriveMotionCommand(TrajectoryGenerator.getInstance().getTrajectorySet().platformToCargoSide, false));
    ...
    addParallel(
        new AutoCameraTrackWhenCrossXBoundary(70, MovingXDirection.Negative, 0.55, Constants.finishedAtCargoLimeY)); // 100
    addSequential(
        new DriveMotionCommand(TrajectoryGenerator.getInstance().getTrajectorySet().cargoBackMidToLoading, false));

    addSequential(new IntakeHatchArms(HatchArmState.IN));
    addSequential(new WaitCommand("Grab Break", .25));
    addSequential(
        new DriveMotionCommand(TrajectoryGenerator.getInstance().getTrajectorySet().loadingToCargoSide, false));
    addSequential(new DriveAbsoluteTurnMP(-90, 240, MPSoftwareTurnType.TANK));
    addSequential(new DrivePathCameraTrackWithVelocity(2, Constants.finishedAtCargoLimeY));
  }
}
